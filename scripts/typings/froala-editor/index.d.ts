 declare module 'froala-editor' {
   ...
   */
   function DefineIcon(name: string, parameters: Partial<DefineIconParameters>): void;

   interface DefineIconParameters {
    /**
   ...
   */
   const ICON_DEFAULT_TEMPLATE: string;
   ...
   *   }
   */
   const ICON_TEMPLATES: GenericObject<string>;
   ...
   * @param parameters
   */
   function RegisterCommand(name: string, parameters: Partial<RegisterCommandParameters>): void;
...
// Froala config defaults
   const DEFAULTS: Partial<FroalaOptions>;
...
// Froala Plugins
   const PLUGINS: GenericObject<(editor: FroalaEditor) => CustomPlugin>;

   function RegisterQuickInsertButton(name: string, parameters: object): void;

   function DefineIconTemplate(name: string, template: string): void;
   ...
   interface EmoticonButton {
    code: string;
    desc: string;
  }

   type UploadMethod = 'POST' | 'PUT';
   type DeleteMethod = 'POST' | 'DELETE';
   type GetMethod = 'POST' | 'GET';
   ...
   interface FroalaEvents {
    ...
    'image.beforeUpload': (images: Image[]) => void;
  }
  ...
  interface Apply<T> {
   ...
   interface FontFamily extends Apply<string> {
  }

   interface FontSize extends Apply<string> {
  }

   type FormatAttributes = { [key: string]: any; };
   ...
   interface InlineClass extends Apply<string> {
  }

   interface InlineStyle extends Apply<string> {
  }
   ...
   interface LineHeight extends Apply<number> {
  }
   ...
   interface ParagraphFormat extends Apply<string> {
  }

   interface ParagraphStyle extends Apply<string> {
  }
   ...
   interface Quote extends Apply<string> {
  }
